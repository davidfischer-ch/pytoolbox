...
class FindMixin(object):
    ...
    def clean_elements(self, elements, criteria, fail=True):
        if elements:
            return elements[0] if len(elements) == 1 else elements
        if fail:
            raise exceptions.NoSuchElementException(criteria)

    def find_css(self, css_selector, fail=True):
        return self.clean_elements(self.find_elements_by_css_selector(css_selector), css_selector, fail)
