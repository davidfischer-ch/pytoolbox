...
class TestEnvironment(base.TestCase):
    ...
    def test_ensure_num_units(self):
        ...
        self.dict_equal(
            environment.ensure_num_units('mysql', 'my_mysql', num_units=2),
            {'deploy_units': None})

        self.dict_equal(
            environment.ensure_num_units('lamp', None, num_units=4),
            {'deploy_units': None})
        ...
        self.dict_equal(
            environment.ensure_num_units('mysql', 'my_mysql', num_units=5),
            {'add_units': None})

        environment.get_units = Mock(return_value=TEST_UNITS_LAMP_4)
        self.dict_equal(
            environment.ensure_num_units(None, 'lamp', num_units=5),
            {'add_units': None})
